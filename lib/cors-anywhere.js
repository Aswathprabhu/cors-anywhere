...
function getHandler(options, proxy) {
  ...
  return function(req, res) {
    ...
    var space = ' ';
    var data = `
      ${introLine.repeat(10)}${space.repeat(3)}Request - ${++requestCount}${space.repeat(3)}${introLine.repeat(10)}
  
      IP: ${req.socket.remoteAddress}
      TimeReceived: ${Date(Date.now())}
      HostName: ${((req.corsAnywhereRequestState || {}).location || {}).hostname}
  
      ${introLine.repeat(29)}
  
    `
    const { logFile = 'logs.txt' } = options
    fs.appendFile(logFile, data, (err) => {
      if (err) {
        return console.log(err);
      }
    });
    // Logging Request Ends
    proxyRequest(req, res, proxy);
  };
}
